...
triangleIntersection::triangleIntersection(double a0, double a1, double a2, double t_)
    : b0(a0), b1(a1), b2(a2), dist(t_), valid(true) {}

triangleIntersection::triangleIntersection()
    : b0(0), b1(0), b2(0), dist(0), valid(false) {}

// ============================================================
// --- triangleMesh ---
triangleMesh::triangleMesh(){}

triangleMesh::triangleMesh(std::shared_ptr<bxdf> mat) : mat(mat) {}

triangleMesh::triangleMesh(const std::vector<vec3h>& verts, const std::vector<int>& inds, int num_tri, std::shared_ptr<bxdf> mat)
    : vertices(verts), indices(inds), num_triangles(num_tri), mat(mat) {}

triangleMesh::triangleMesh(const std::vector<vec3h>& verts, const std::vector<int>& inds, int num_tri)
...
triangle::triangle(triangleMesh* triMesh, int mesh_index, std::shared_ptr<bxdf> material)
    : mesh(triMesh), mesh_index(mesh_index), mat(material) {}

triangle::triangle(triangleMesh* triMesh, int mesh_index)
    : mesh(triMesh), mesh_index(mesh_index), mat(std::make_shared<diffuseBXDF>(color(0.5, 0.5, 0.5, 0))) {}

triangle::triangle() : mesh(nullptr), mesh_index(-1), mat(std::make_shared<diffuseBXDF>(color(0.5, 0.5, 0.5, 0))) {}
