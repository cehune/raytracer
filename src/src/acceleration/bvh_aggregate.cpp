...
BVHAggregate::BVHAggregate() : max_prims_in_node(1), head(nullptr) {}

BVHAggregate::BVHAggregate(std::vector<std::shared_ptr<hittable>> objs, int max_prims)
    : max_prims_in_node(max_prims) {
    std::vector<BVHPrimitive> bvhPrimitives(objs.size());
    for (size_t i = 0; i < objs.size(); ++i) {
        bvhPrimitives[i] = BVHPrimitive(i, (*objs[i]).bounds(), objs[i]);
    }
    if (!objs.empty()) {
...
BVHTreeNode* BVHAggregate::get_head() const {
    return head.get();
}


std::unique_ptr<BVHTreeNode> BVHAggregate::sah_recursive(std::vector<BVHPrimitive>& bvhPrimitives) {
